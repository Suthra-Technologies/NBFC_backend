...
exports.createMember = async (req, res, next) => {
    try {
        const bankId = req.user.bankId || req.tenant?._id;
        let branchId = req.user.branchId || req.get("x-branch-id") || req.body.branchId;

        if (!bankId || !branchId) {
            return res.status(400).json({
                success: false,
                message: "User must be assigned to a bank and branch",
                details: { bankId: !!bankId, branchId: !!branchId }
            });
        }
...
exports.getAllMembers = async (req, res, next) => {
    try {
        ...
        const bankId = req.user.bankId || req.tenant?._id;
        const members = await memberService.getAllMembers(bankId, query);
...
exports.getMemberById = async (req, res, next) => {
    try {
        const bankId = req.user.bankId || req.tenant?._id;
        const member = await memberService.getMemberById(req.params.memberId, bankId);
...
exports.updateMember = async (req, res, next) => {
    try {
        const bankId = req.user.bankId || req.tenant?._id;
        const member = await memberService.updateMember(req.params.memberId, req.body, bankId);
...
exports.deleteMember = async (req, res, next) => {
    try {
        const bankId = req.user.bankId || req.tenant?._id;
        const member = await memberService.deleteMember(req.params.memberId, bankId);
